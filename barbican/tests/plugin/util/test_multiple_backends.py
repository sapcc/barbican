...
class MockedManager(object):
    ...
    def __init__(
        self, names, enabled=True, plugin_lookup_field="store_plugin"
    ):
...
class WhenReadingMultipleBackendsConfig(test_utils.MultipleBackendsTestCase):
    ...
    def test_successful_conf_read(self):
        ...
        self.init_via_conf_file(
            ss_plugins, cr_plugins, enabled=True, global_default_index=1
        )
    ...
    def test_fail_when_store_plugin_name_missing(self):
        ...
        self.assertRaises(
            exception.MultipleStorePluginValueMissing,
    ...
    def test_fail_when_global_default_not_specified(self):
        ...
        self.assertRaises(
            exception.MultipleStoreIncorrectGlobalDefault,
            multiple_backends.read_multiple_backends_config,
        )
    ...
    def test_fail_when_stores_lookup_suffix_missing_when_enabled(self):
        ...
        cr_plugins = ["cr_p1", "cr_p2", "cr_p3"]
        self.init_via_conf_file(
            ss_plugins, cr_plugins, enabled=True, global_default_index=0
        )

        conf = config.get_module_config("secretstore")
        conf.set_override("stores_lookup_suffix", [], group="secretstore")
        self.assertRaises(
            exception.MultipleSecretStoreLookupFailed,
            multiple_backends.read_multiple_backends_config,
        )
    ...
    def test_fail_when_secretstore_section_missing(self):
        ...
        ss_conf.set_override(
            "stores_lookup_suffix", existing_value, "secretstore"
        )
...
class WhenInvokingSyncSecretStores(test_utils.MultipleBackendsTestCase):
    ...
    def test_successful_syncup_no_existing_secret_stores(self):
        ...
        default_secret_store = (
            multiple_backends.get_global_default_secret_store()
        )
    ...
    def test_syncup_with_existing_secret_stores(self):
        ...
        self.assertEqual(
            f"{MockedManager.NAME_PREFIX}ss_p2 [ss_p2]", ss_db_entry.name
        )
        ...
        self.assertEqual(
            f"{MockedManager.NAME_PREFIX}cr_p3 [cr_p3]",
    ...
    def test_syncup_modify_global_default(self):
        ...
        self.assertEqual(
            f"{MockedManager.NAME_PREFIX}cr_p1 [cr_p1]",
        ...
        self.assertEqual(
            f"{MockedManager.NAME_PREFIX}cr_p9 [cr_p9]",
            global_secret_store.name,
        )
    ...
    def test_syncup_delete_secret_store_with_preferred_project_using_it(self):
        """Test removing secret store when defined as preferred.

        Removing secret store will fail if it's defined as the
        preferred store.
        """
        ss_plugins = ["ss_p1", "ss_p2", "ss_p3", "ss_p4"]
        ...
        with mock.patch(
            ...
            self.assertRaises(
                exception.MultipleStorePluginStillInUse,
                multiple_backends.sync_secret_stores,
                secretstore_manager,
                crypto_manager,
            )
...
class TestGetApplicablePlugins(test_utils.MultipleBackendsTestCase):
    ...
    def test_get_when_project_preferred_plugin_is_set(self):
        ...
        with mock.patch(
            "barbican.model.repositories.ProjectSecretStoreRepo."
            "get_secret_store_for_project"
        ) as pref_func:
    ...
    def test_get_when_project_preferred_plugin_is_not_found_in_conf(self):
        ...
        with mock.patch(
            ...
            self.assertRaises(
                exception.MultipleStorePreferredPluginMissing,
    ...
    def test_get_when_project_preferred_plugin_not_set_then_default_used(self):
        ...
        cr_manager = MockedManager(
            cr_plugins, plugin_lookup_field="crypto_plugin"
        )
        project_id = uuidutils.generate_uuid(dashed=False)
        ...
        with mock.patch(
            ...
            objs = multiple_backends.get_applicable_crypto_plugins(
                cr_manager, project_id, None
            )
    ...
    def test_get_applicable_store_plugins_when_multiple_backend_not_enabled(
        ...
        self.init_via_conf_file(
            ss_conf_plugins, cr_conf_plugins, enabled=False
        )
        ss_manager = MockedManager(ss_plugins)

        ss_config.set_override(
            "enabled_secretstore_plugins", ss_plugins, group="secretstore"
        )

        objs = multiple_backends.get_applicable_store_plugins(
            ss_manager, None, None
        )
...
class TestPluginsGenerateStoreAPIMultipleBackend(
    ...
    def _create_project(self):
        ...
        project.external_id = "keystone_project_id" + uuidutils.generate_uuid(
            dashed=False
        )
    ...
    def test_no_preferred_default_plugin(self, dataset):
        ...
        self.init_via_conf_file(
            dataset["store_plugins"], dataset["crypto_plugins"], enabled=True
        )

        with mock.patch(
            "barbican.plugin.crypto.p11_crypto.P11CryptoPlugin."
            "_create_pkcs11"
        ...
        self.assertIsInstance(plugin_found, dataset["default_store_class"])
        ...
        if dataset["default_crypto_class"]:
            ...
            self.assertIsInstance(
                crypto_plugin, dataset["default_crypto_class"]
            )

            # With the modified friendly name format, we need to check that the
            # global_secret_store.name follows the expected pattern:
            # "friendly_name [plugin_id]" The friendly name comes from the
            # crypto plugin's get_plugin_name() method but we can't directly
            # compare since the friendly name in the DB also
            # includes the plugin ID
            plugin_name = crypto_plugin.get_plugin_name()
            self.assertTrue(
                global_secret_store.name.startswith(plugin_name + " [")
            )
            self.assertTrue(global_secret_store.name.endswith("]"))
        else:  # crypto class is not used
            # With the modified friendly name format, we need to check
            # that the global_secret_store.name follows the expected pattern:
            # "friendly_name [plugin_id]" The friendly name
            #  comes from the store plugin's get_plugin_name() method but
            # we can't directly compare since the friendly name in the DB
            # also includes the plugin ID
    ...
    def test_project_preferred_default_plugin(self, dataset):
        ...
        pkcs11_secret_store = self._get_secret_store_entry(
            "store_crypto", "p11_crypto"
        )
        kmip_secret_store = self._get_secret_store_entry("kmip_plugin", None)
        db_secret_store = self._get_secret_store_entry(
            "store_crypto", "simple_crypto"
        )
        ...
        # specific
        plugin_found = manager.get_plugin_store(
            keySpec, project_id=project1.id
        )
        self.assertIsInstance(
            plugin_found, store_crypto.StoreCryptoAdapterPlugin
        )
        crypto_plugin = cm.get_manager().get_plugin_store_generate(
        ...
        plugin_found = manager.get_plugin_store(
            keySpec, project_id=project2.id
        )
        self.assertIsInstance(plugin_found, kss.KMIPSecretStore)

        self.assertRaises(
            base.CryptoPluginNotFound,
            cm_manager.get_plugin_store_generate,
            base.PluginSupportTypes.ENCRYPT_DECRYPT,
        ...
        # backend specific
        plugin_found = manager.get_plugin_store(
            keySpec, project_id=project3.id
        )
        self.assertIsInstance(
            plugin_found, store_crypto.StoreCryptoAdapterPlugin
        )
        ...
        plugin_found = manager.get_plugin_store(
            keySpec, project_id=project4.id
        )
        self.assertIsInstance(plugin_found, dataset["default_store_class"])
